...
const assignmentApi = async (id) => {
	if (id != null) {
		try {
			...
			state.formData.content.submitted = data.submitted;
			if (data.submissionFile == null) {
				state.formData.content.submissionFile = [];
			} else {
				state.formData.content.submissionFile = [data.submissionFile];
			}
