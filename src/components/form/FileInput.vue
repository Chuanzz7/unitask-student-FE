...
<template>
  <div>
    <div class="w-full">
      <label class="w-full ml-1 text-left font-bold text-xs text-slate-700 dark:text-white/80">{{ props.label }}</label>
    ...
    <div v-for="(file, index) in modelValue" :key="index"
      ...
      <button class="underline font-semibold text-blue-500" @click="openNewTab(file.url)">{{ file.fileName }}</button>
